...
class SharedService {
  ...
  static Future<void> setusername(
      UpdateuserResponseModel loginResponse,
      ) async {
    var isCacheKeyExist = await APICacheManager().isAPICacheKeyExist("login_details");
    if (isCacheKeyExist) {
      var cacheModel = await APICacheManager().getCacheData("login_details");
      Map<String, dynamic> syncData = jsonDecode(cacheModel.syncData);
      Map<String, dynamic> userData = syncData['data'];
      userData['name'] = loginResponse.name;
      String updatedSyncData = jsonEncode(syncData);
      cacheModel.syncData = updatedSyncData;
      await APICacheManager().addCacheData(cacheModel);
    }
